<?php
/* Smarty version 3.1.30, created on 2017-11-13 11:49:21
  from "/Users/gaoxin/Documents/www/1703/mvc/template/index/index.html" */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.30',
  'unifunc' => 'content_5a0978b10cf4a4_68104542',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    'dff353243cfa551ac8be155ceb6aeffecc79f78e' => 
    array (
      0 => '/Users/gaoxin/Documents/www/1703/mvc/template/index/index.html',
      1 => 1510569762,
      2 => 'file',
    ),
  ),
  'includes' => 
  array (
  ),
),false)) {
function content_5a0978b10cf4a4_68104542 (Smarty_Internal_Template $_smarty_tpl) {
$_smarty_tpl->_subTemplateRender($_smarty_tpl->tpl_vars['header']->value, $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, 0, $_smarty_tpl->cache_lifetime, array(), 0, true);
?>

<link rel="stylesheet" href="<?php echo CSS_URL;?>
/wyp/index.css">
<?php echo '<script'; ?>
 src="<?php echo JS_URL;?>
/wyp/velocity.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="<?php echo JS_URL;?>
/wyp/shutter.js"><?php echo '</script'; ?>
>
<?php echo '<script'; ?>
 src="<?php echo JS_URL;?>
/wyp/index.js"><?php echo '</script'; ?>
>
<!--bann er-->
<section class="banner">
    <main>
        <div class="img">
            <img src="<?php echo IMG_URL;?>
/3 (1).jpg" alt="">
        </div>
    </main>
    <section class="aside">
        <h3 class="icon-xiaoxizhongxinsuifang"> <span>随心Get</span></h3>
        <a href="index.php?m=index&f=type">♥ 新上榜<span>&#xe606;</span></a>
        <a href="index.php?m=index&f=get&a=hot">♥ 7日热门<span>&#xe606;</span></a>
        <a href="index.php?m=index&f=type">♥ 30日推荐<span>&#xe606;</span></a>
        <h3 class="icon-fenlei1"> <span>分类Type</span></h3>

        <?php
$_from = $_smarty_tpl->smarty->ext->_foreach->init($_smarty_tpl, $_smarty_tpl->tpl_vars['positions']->value, 'v');
if ($_from !== null) {
foreach ($_from as $_smarty_tpl->tpl_vars['v']->value) {
?>
        <a href="index.php?m=index&f=lists&a=show&pid=<?php echo $_smarty_tpl->tpl_vars['v']->value['pid'];?>
"><?php echo $_smarty_tpl->tpl_vars['v']->value["pname"];?>
</a>
        <?php
}
}
$_smarty_tpl->smarty->ext->_foreach->restore($_smarty_tpl);
?>

        <h3 class="icon-icon-test"> <span>推荐Author</span></h3>
        <a href="index.php?m=index&f=author" class="test1">
            <img src="<?php echo IMG_URL;?>
/3 (1).jpg" alt="">
            <div class="test">啊啊 &nbsp;&nbsp;&nbsp;&#xe64f;</div>
        </a>
        <a href="index.php?m=index&f=author" class="test1"><img src="<?php echo IMG_URL;?>
/3 (1).jpg" alt="">
            <div class="test">哎呀 &nbsp;&nbsp;&nbsp;&#xe64f;</div></a>
        <a href="index.php?m=index&f=author" class="test1"><img src="<?php echo IMG_URL;?>
/3 (1).jpg" alt="">
            <div class="test">哎呦 &nbsp;&nbsp;&nbsp;&#xe64f;</div></a>
        <a href="index.php?m=index&f=author" class="test1"><img src="<?php echo IMG_URL;?>
/3 (1).jpg" alt="">
            <div class="test">哎哎 &nbsp;&nbsp;&nbsp;&#xe64f;</div></a>
        <a href="index.php?m=index&f=care" class="test2"><span>更多+</span></a>
    </section>
</section>
<section class="contenta">
    <main>
        <div class="list">
            <div class="list1 icon-plumage"> <span>最新动态·Recent News</span></div>
        </div>
        <div class="content1">
            <div class="item2">
            <div class="item"></div>
            <div class="item1"></div>
            <div class="content2">
                <div class="content3">
                    <a href="index.php?m=index&f=author" class="content31"><img src="<?php echo IMG_URL;?>
/touxiang.jpg" alt=""></a>
                    <a href="index.php?m=index&f=author" class="content32">陌上花开</a>
                    <span>3小时前</span>
                </div>
                <div class="content4">
                    <div class="content41">
                        <a href="index.php?m=index&f=content">你愿意爱我吗？</a>
                    <span>我只希望这一程，你能活得烈马青葱，用你喜欢的模样，过你喜欢的生活。手里拉着无数条连着线的风筝，只要风在，风筝就会一直飞上天。﻿﻿当命运需要我们逆风飞翔的时候，请不要随风...</span>
                    </div>
                    <div class="content42"><img src="<?php echo IMG_URL;?>
/11.png" alt=""></div>
                </div>
                <div class="content5">
                    <a href="index.php?m=index&f=type" class="content51">谈情说爱</a>
                    <a href="index.php?m=index&f=content" class="content52 icon-liulan3"> 521</a>
                    <a href="index.php?m=index&f=content" class="content53 icon-comment"> 215</a>
                    <div class="content54 icon-xiai1"> 125</div>
                    <div class="content55 icon-qian1"> 1</div>
                </div>
            </div>
            </div>
            <div class="item2">
            <div class="item"></div>
            <div class="item1"></div>
            <div class="content2">
                <div class="content3">
                    <a href="index.php?m=index&f=author" class="content31"><img src="<?php echo IMG_URL;?>
/touxiang.jpg" alt=""></a>
                    <a href="index.php?m=index&f=author" class="content32">陌上花开</a>
                    <span>3小时前</span>
                </div>
                <div class="content4">
                    <div class="content41">
                        <a href="index.php?m=index&f=content">你愿意爱我吗？</a>
                        <span>我只希望这一程，你能活得烈马青葱，用你喜欢的模样，过你喜欢的生活。手里拉着无数条连着线的风筝，只要风在，风筝就会一直飞上天。﻿当命运需要我们逆风飞翔的时候，请不要随风...</span>
                    </div>
                    <div class="content42"><img src="<?php echo IMG_URL;?>
/11.png" alt=""></div>
                </div>
                <div class="content5">
                    <a href="index.php?m=index&f=type" class="content51">谈情说爱</a>
                    <a href="index.php?m=index&f=content" class="content52 icon-liulan3"> 521</a>
                    <a href="index.php?m=index&f=content" class="content53 icon-comment"> 215</a>
                    <div class="content54 icon-xiai1"> 125</div>
                    <div class="content55 icon-qian1"> 1</div>
                </div>
            </div>
            </div>
            <div class="item2">
            <div class="item"></div>
            <div class="item1"></div>
            <div class="content2">
                <div class="content3">
                    <a href="index.php?m=index&f=author" class="content31"><img src="<?php echo IMG_URL;?>
/touxiang.jpg" alt=""></a>
                    <a href="index.php?m=index&f=author" class="content32">陌上花开</a>
                    <span>3小时前</span>
                </div>
                <div class="content4">
                    <div class="content41">
                        <a href="index.php?m=index&f=content">你愿意爱我吗？</a>
                        <span>我只希望这一程，你能活得烈马青葱，用你喜欢的模样，过你喜欢的生活。手里拉着无数条连着线的风筝，只要风在，风筝就会一直飞上天。﻿当命运需要我们逆风飞翔的时候，请不要随风...</span>
                    </div>
                    <div class="content42"><img src="<?php echo IMG_URL;?>
/11.png" alt=""></div>
                </div>
                <div class="content5">
                    <a href="index.php?m=index&f=type" class="content51">谈情说爱</a>
                    <a href="index.php?m=index&f=content" class="content52 icon-liulan3"> 521</a>
                    <a href="index.php?m=index&f=content" class="content53 icon-comment"> 215</a>
                    <div class="content54 icon-xiai1"> 125</div>
                    <div class="content55 icon-qian1"> 1</div>
                </div>
            </div>
            </div>
            <div class="item2">
            <div class="item"></div>
            <div class="item1"></div>
            <div class="content2">
                <div class="content3">
                    <a href="index.php?m=index&f=author" class="content31"><img src="<?php echo IMG_URL;?>
/touxiang.jpg" alt=""></a>
                    <a href="index.php?m=index&f=author" class="content32">陌上花开</a>
                    <span>3小时前</span>
                </div>
                <div class="content4">
                    <div class="content41">
                        <a href="index.php?m=index&f=content">你愿意爱我吗？</a>
                        <span>我只希望这一程，你能活得烈马青葱，用你喜欢的模样，过你喜欢的生活。手里拉着无数条连着线的风筝，只要风在，风筝就会一直飞上天。﻿当命运需要我们逆风飞翔的时候，请不要随风...</span>
                    </div>
                    <div class="content42"><img src="<?php echo IMG_URL;?>
/11.png" alt=""></div>
                </div>
                <div class="content5">
                    <a href="index.php?m=index&f=type" class="content51">谈情说爱</a>
                    <a href="index.php?m=index&f=content" class="content52 icon-liulan3"> 521</a>
                    <a href="index.php?m=index&f=content" class="content53 icon-comment"> 215</a>
                    <div class="content54 icon-xiai1"> 125</div>
                    <div class="content55 icon-qian1"> 1</div>
                </div>
            </div>
            </div>
            <div class="item2">
            <div class="item"></div>
            <div class="item1"></div>
            <div class="content2">
                <div class="content3">
                    <a href="index.php?m=index&f=author" class="content31"><img src="<?php echo IMG_URL;?>
/touxiang.jpg" alt=""></a>
                    <a href="index.php?m=index&f=author" class="content32">陌上花开</a>
                    <span>3小时前</span>
                </div>
                <div class="content4">
                    <div class="content41">
                        <a href="index.php?m=index&f=content">你愿意爱我吗？</a>
                        <span>我只希望这一程，你能活得烈马青葱，用你喜欢的模样，过你喜欢的生活。手里拉着无数条连着线的风筝，只要风在，风筝就会一直飞上天。﻿当命运需要我们逆风飞翔的时候，请不要随风...</span>
                    </div>
                    <div class="content42"><img src="<?php echo IMG_URL;?>
/11.png" alt=""></div>
                </div>
                <div class="content5">
                    <a href="index.php?m=index&f=type" class="content51">谈情说爱</a>
                    <a href="index.php?m=index&f=content" class="content52 icon-liulan3"> 521</a>
                    <a href="index.php?m=index&f=content" class="content53 icon-comment"> 215</a>
                    <div class="content54 icon-xiai1"> 125</div>
                    <div class="content55 icon-qian1"> 1</div>
                </div>
            </div>
            </div>
            <a href="" class="more icon-liulan"> &nbsp;&nbsp;查看更多</a>
        </div>
    </main>
</section>
<?php $_smarty_tpl->_subTemplateRender($_smarty_tpl->tpl_vars['footer']->value, $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, 0, $_smarty_tpl->cache_lifetime, array(), 0, true);
}
}
